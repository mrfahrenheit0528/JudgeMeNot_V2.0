...
def api_detail(event_id):
    ...
    try:
        ...
        for c in event.contestants:
            ...
            final_score = 0.0
            segment_scores = {}
            segment_weighted = {} # Tracks the internal weighted math
            ...
            for seg in event.segments:
                ...
                if event.event_type == 'Point-Based':
                    ...
                    # THE FIX: Split display (100%) vs weighted calculation
                    segment_scores[seg.id] = avg_seg_score
                    segment_weighted[seg.id] = weighted_score
        ...
        for cat, c_list in contestants_data.items():
            ...
            for i, c in enumerate(c_list):
                ...
                if event.event_type == 'Point-Based':
                    ...
                    for sid, val in c['segment_weighted'].items():
                        if isinstance(val, (int, float)):
                            c['segment_weighted'][sid] = str(int(val)) if float(val).is_integer() else str(val)
                else:
                    ...
                    for sid, val in c['segment_weighted'].items():
                        if isinstance(val, (int, float)):
                            c['segment_weighted'][sid] = f"{val:.2f}"
